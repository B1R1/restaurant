...
public class HMenuDao implements MenuDao {
    ...
    public int deleteMenu(String name) {
        ...
        session.beginTransaction();
        try {
            session.delete(menuDao.deleteMenu(name));
            session.getTransaction().commit();
            System.out.println("Menu deleted from DB");
        }catch (RuntimeException e){
            System.out.println("No Menu in DB to delete");
        }
        return 1;
    }
    ...
    public void addDish(String menuName, String dishName) {
        Set<Dishes> allDishesInMenu = new HashSet<>(menuDao.getMenuByName(menuName).getDishes());
